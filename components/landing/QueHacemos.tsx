...
function SprintersPanel({ tab }: { tab: any }) {
    ...
    <div className="qh-panel qh-panel--sprinters">
      ...
      <div className="qh-sprint-grid">
        ...
        <div className="qh-sprint-right">
          ...
          <ul className="qh-sprint-chips">
            {prog.list.map((item: string) => <li key={item} className="qh-sprint-chip">{item}</li>)}
          </ul>
        </div>
      </div>
      <div className="qh-sprint-benefits">
        ...
        <div className="qh-sprint-benefits__grid">
          {prog.benefits.map((b: { title: string; desc: string }) => (
      ...
      {tab.comingSoon && <p className="qh-coming-soon">{tab.comingSoon}</p>}
    </div>
  )
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function AdLabPanel({ tab }: { tab: any }) {
  return (
    <div className="qh-panel qh-panel--adlab">
      ...
      <div className="qh-how">
        ...
        <div className="qh-how__grid">
          {tab.howWeWork.items.map((item: { title: string; desc: string }) => (
      ...
      <div className="qh-programs">
        ...
        <div className="qh-programs__grid">
          {tab.programs.items.map((prog: { name: string; desc: string }) => (
      ...
      <div className="qh-ctas">
        {tab.ctas.map((cta: { label: string; href: string; variant: string }) => (
...
function ComunidadPanel({ tab }: { tab: any }) {
  return (
    <div className="qh-panel qh-panel--comunidad">
      ...
      <a href={tab.ctaHref} className="btn btn--purple">{tab.cta}</a>
    </div>
  )
}
...
export default function QueHacemos() {
  ...
  return (
    <section className="que-hacemos" ref={ref as React.RefObject<HTMLElement>}>
      <div className="container">
        <div className="que-hacemos__header anim-ready" style={{ '--delay': '0s' } as React.CSSProperties}>
        ...
        <div className="que-hacemos__tabbar anim-ready" style={{ '--delay': '0.1s' } as React.CSSProperties}>
          {t.tabs.map((tab, i) => (
            <button
              key={tab.key}
