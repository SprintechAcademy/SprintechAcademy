  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div className="text-center">
        {enrolled && (
          <div className="mb-5 px-4 py-3 rounded-xl bg-lime/10 border border-lime/20 text-sm text-lime leading-snug">
            ¡Pago exitoso! Ingresa tu correo para acceder.
          </div>
        )}

        <h1 className="font-bold tracking-tight leading-[1.15] mb-3">
          <span className="block text-white" style={{ fontSize: 'clamp(1.75rem, 5vw, 2.5rem)' }}>Accede a la</span>
          <span className="block text-lime" style={{ fontSize: 'clamp(1.75rem, 5vw, 2.5rem)' }}>plataforma.</span>
        </h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        >
          {loading ? (
            <>
              <span className="w-3.5 h-3.5 border-2 border-carbon/30 border-t-carbon rounded-full animate-spin" />
              Enviando...
            </>
          ) : (
            'Enviar código'
      ...
      <div className="flex flex-col items-center gap-2 text-center">
        <p className="text-xs text-white/30">Acceso vía código de un solo uso · sin contraseña</p>
        <p className="text-xs text-white/22">¿No tienes cuenta? Inscríbete en un reto o programa.</p>
        <a href="/" className="text-xs text-white/28 hover:text-white/55 transition-colors duration-200 mt-1">
          ← Volver al inicio
        </a>
      </div>
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center px-6 py-20 bg-carbon overflow-hidden">
      <div
        ...
        style={{ background: 'radial-gradient(ellipse 55% 40% at 50% 35%, rgba(139,92,246,0.07) 0%, transparent 65%)' }}
      />
      <div className="relative w-full max-w-sm flex flex-col gap-8">
        <div className="flex flex-col items-center gap-3">
          <Logo href="/" />
          <p className="text-[10px] tracking-widest uppercase text-white/28 font-medium" style={{ fontFamily: 'var(--font-syne)' }}>
